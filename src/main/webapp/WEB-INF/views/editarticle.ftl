...
<body>
    <form name="editblog" method="post" action="saveEditArticle">
      <table>
       <tr>
       ...
       <tr>
         <th>内容</th>
         <th><textarea rows="25" cols="100" name="artContent" value="${article.artContent?if_exists}"  >${article.artContent?if_exists}</textarea>
         </th>
       </tr>
  <#--  
       <tr align="left">
            ...
            <select name="artCatagory">
			  <#if catagoriesList??&&(catagoriesList?size>0)>
                <#list catagoriesList as catagories>
                   <option value ="${catagories.cataName?if_exists}" <#if (article.artCatagory??)&&(article.artCatagory==catagories.cataName)>selected="selected" </#if> >${catagories.cataName?if_exists}</option>
                </#list>
  			 </#if> 
         </th>
        </tr>
